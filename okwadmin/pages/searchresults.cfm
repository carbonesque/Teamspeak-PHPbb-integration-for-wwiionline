...
<!--- Condition for finding users not in any group --->
<cfif #url.search# EQ 'LOST'>
	<cfquery name="search" datasource="OKWTS">
		SELECT * FROM ts2_tsman_users 
		WHERE group_key = ''
		ORDER BY group_admin DESC, s_client_name ASC;
	</cfquery>
<cfelse>
<!--- Generic find users query.  May need some work as I think its easily hackable --->
	<cfquery name="search" datasource="OKWTS">
		SELECT * FROM ts2_tsman_users 
		WHERE #url.type# LIKE '%#url.search#%'
		<cfif #sessioninfo.group_admin# GT '0'>
			AND group_key LIKE '#sessioninfo.group_key#%' 
		<cfelse>
		</cfif> 
...
<cfoutput>
	...
	<table class="size11grey" bgcolor="##555555" align="left" width="425" cellpadding="1">
	  ...
	  <cfloop query="search" startrow="#StartRow_search#" endrow="#EndRow_search#">
		...
		<tr>
		  <td width="60" align="left" bgcolor="#color#" class="size9grey"><a href="http://okw.wwiiol.net:8500/okwadmin/pages/admin/edituser.cfm?name=#search.s_client_name#"><strong><u>#search.s_client_name#</u></strong></a></td>
